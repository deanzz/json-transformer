...
trait PCAJsonComponent extends JsonPluginComponent {

  class PCAJson extends JsonPlugin[OldSPCAParam] {
    override def fromOld(json: JSONObject): OldSPCAParam = {
      ...
      OldSPCAParam(segment, dimension, accumulateRate, K, dimColumns, categorySplit)
    }

    override def toNew(oldObj: OldSPCAParam): String = {
  ...
  case class OldSPCAParam(segment: Option[Double],

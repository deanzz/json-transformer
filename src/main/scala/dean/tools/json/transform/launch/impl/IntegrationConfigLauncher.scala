...
class IntegrationConfigLauncher extends LaunchComponent[IntegrationConfigParam, BulkWriteResult]
  with MongoIntegrationConfigDaoComponent with Log4jLoggerComponent {
  override val dao = new IntegrationConfigDao
  ...
  override def workResultHandler(seq: Seq[(IntegrationConfigParam, String)]): Unit = {
    ...
    logger.info(s"IntegrationConfig: Elapsed time = ${finished - start}ms, MatchedCount = ${result.getMatchedCount}, ModifiedCount = ${result.getModifiedCount}, DeletedCount = ${result.getDeletedCount}, InsertedCount = ${result.getInsertedCount}")
